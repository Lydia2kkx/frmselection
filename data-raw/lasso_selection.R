...
n <- length(coefi) #n is the number of the coefficients
...
coefi.new <- coefi[-index.tau] #the new coeffients doesn't contain the penalized parameters
threshold <- 1e-3 #Users can set this threshold as whatever they like. The default value is 1e-3
...
lasso.select <- function(coefficent, threshold = 1e-3){
  n <- length(coefficent)
  ncommon <- length(x[,-ind])
  nfactor <- length(levels(x[,ind]))
  tau.common <- coefi[ncommon+1]
  tau.factor <- coefi[ncommon+nfactor+1]
  lambda.common <- 1/tau.common
  names(lambda.common) <- "mu.s.la(f1).lambda"
  ...
  names(lambda.factor) <- "mu.s.la(f2).lambda"
  index.tau <- c(ncommon+1, ncommon+nfactor+1)
  coefi.new <- coefficent[-index.tau]
  lasso.index <- NULL
  for(i in 1:length(coefi.new)){
    if(abs(coefi.new[i]) < threshold){
      coefi.new[i] = 0
